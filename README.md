...
- 支持大画面清晰度切换、画中画播放、单路全屏和统一静音控制；`4 大四宫格` 默认使用 `720p/middle`
- 使用 `headimg` 渲染视角头像；缺失头像时自动使用视角名称占位
- 小窗支持实时预览、智能省流和头像占位三种策略；智能省流只为可见小窗拉低清流，减少多路监看时的卡顿
- 支持红方优先、蓝方优先的注意力排序，把对应阵营的小窗视角排到前面
- 支持纯净模式，隐藏控制区、标题、卡片文字和播放器控件，按当前布局把直播画面密铺到整个视口
- 支持赛区切换、视角筛选、手动刷新和 GitHub 仓库跳转
...
DJI 的 HLS 链接带有 `auth_key`，链接可能会过期或被更新。页面会在首次进入时读取 JSON；如果直播源状态变化或链接失效，可以点击刷新获取最新完整 URL。

如果直播加载失败，通常有三类原因：

- 当前赛区或视角还未开播。
- `auth_key` 已失效，点击刷新获取最新链接。
- 当前网络无法访问 DJI CDN。
